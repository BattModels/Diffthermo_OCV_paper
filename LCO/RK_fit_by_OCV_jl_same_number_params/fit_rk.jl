...
# data preprocessing
filename = string("LCO_Carlier2012JES_Fig4a.csv")
df = DataFrame(CSV.File(filename, header=false))
# xs = 1.0 .- (df[:,1]/169.97) # filling fraction
SOC = 1.0 .- df[:,1]  # SOC, filling fraction is SOC
xs = 1.0 .- SOC
OCV_true = df[:,2] 
# # For LFP, filling fraction x = 1 - SOC
# # OCV = U0 + RT/nF log((1-x)/x) + RK_terms
...
# fit 
rk_order = 10 # diffthermo: 7th order RK (8 RK params), 1 miscibility gap detected (additional 3 params), 7+3=10
