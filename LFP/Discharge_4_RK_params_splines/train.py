...
is_print = False
_eps = 1e-7
delta_x_spline = 0.03 # for spline interpolation 
...
alpha_miscibility = 1.0 # whether miscibility region is less valued 
...
while loss > 0.01 and epoch < 8000:
    ...
    if epoch % 100 == 0:
        ...
        if x_alpha < x_beta:
            ...
            is_spline = True
            # left side spline
            a_s_left, x_a_left, x_b_left = spline_smoothing(x_alpha, x, ct_pred, GibbsFE, params_list, T = 300, at_which_side = 'left', delta_x = delta_x_spline)
            # right side spline
            a_s_right, x_a_right, x_b_right = spline_smoothing(x_beta, x, ct_pred, GibbsFE, params_list, T = 300, at_which_side = 'right', delta_x = delta_x_spline)
            if a_s_left == None or a_s_right == None:
                is_spline = False
        else:
