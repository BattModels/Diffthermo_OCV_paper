...
G0_start = -334841.5312
Omega0_start = 102896.0234 
Omega1_start = 41539.7734 
Omega2_start = -237956.1094 
Omega3_start = -117621.7891 
...
while epoch < 0:
    ...
    mu_pred = []
    dmu_dx = [] # i.e. dVdQ, differential voltage analysis
    for i in range(0, len(x)):
        x_now = x[i]
        mu_now = mu[i]
        x_now = x_now.requires_grad_()
    ...
    error = np.abs(U_pred_after_ct[-10:] - U_true_value[-10:])
    text = "Leaveout point: RK predicts %.4f, real value %.4f, error %.4f" %(U_pred_after_ct[-1], U_true_value[-1], np.abs(U_pred_after_ct[-1]-U_true_value[-1]))
    print(text)
    with open("result.txt",'w') as fout:
        fout.write("Experimental value on last 10 datapoints: \n")
        for item in U_true_value[-10:]:
            fout.write("%.4f  " %(item))
        fout.write("\n")
        fout.write("Model prediction on last 10 datapoints: \n")
        for item in U_pred_after_ct[-10:]:
            fout.write("%.4f  " %(item))
        fout.write("\n")
