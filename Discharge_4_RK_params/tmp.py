...
while epoch < 0:
    ...
    U_pred_before_ct = -mu_pred*1.0
    plt.plot(1-SOC, U_pred_before_ct, 'k--', label="Before CT Construction", markersize=1.5)
    ...
    # OCV prediction
    U_pred_after_ct = -mu_pred_after_ct*1.0
    plt.plot(1-SOC, U_pred_after_ct, 'r-', label="After CT Construction", markersize=1.5)
    plt.legend(fontsize=12) 
    plt.xlim([0.0,1.0])
    # plt.ylim([1.5, 5.0])
    plt.xlabel("x", fontsize=14)
    plt.xticks(fontsize=14)
    plt.ylabel("Chemical Potential", fontsize=14)
    plt.yticks([])
    plt.legend()
    fig_name = "tmp.png" 
