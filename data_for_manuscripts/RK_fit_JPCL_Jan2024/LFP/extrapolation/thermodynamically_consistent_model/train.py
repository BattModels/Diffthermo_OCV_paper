...
print("Training Complete.\n Predict on the leaveout point")
x_now = torch.from_numpy(np.array([x_leaveout]).astype('float32'))
x_now = x_now.requires_grad_()
g_now = GibbsFE(x_now, params_list, T=300)
mu_pred_now = autograd.grad(outputs=g_now, inputs=x_now, create_graph=True)[0]
U_pred = mu_pred_now.detach().numpy()/(-96485)
text = "Leaveout point: RK predicts %.4f, real value %.4f, error %.4f" %(U_pred, U_leaveout, np.abs(U_leaveout-U_pred))
print(text)
