...
# data preprocessing
filename = string("Discharge_NMat_Fig1a.csv")
df = DataFrame(CSV.File(filename, header=false))
# xs = 1.0 .- (df[:,1]/169.97) # filling fraction
SOC = df[:,1] .- 1.0  # SOC, filling fraction is SOC
xs =  SOC
OCV_true = df[:,2] 
# # For LFP, filling fraction x = 1 - SOC
# # OCV = U0 + RT/nF log((1-x)/x) + RK_terms
...
# fit
rk_order = 8 # diffthermo: 5th order RK (6 RK params),1 miscibility gap (additional 3 params), 5+3 =8
