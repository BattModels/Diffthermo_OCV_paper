...
# error plot on log scale
plt.figure(figsize=(5.5,4))
ax = plt.gca()
plt.plot(SOC_diffthermo,np.abs(OCV_diffthermo-OCV_true),'r-.',label="This work", markersize=4)
# plt.plot(SOC_diffthermo_splines,OCV_diffthermo_splines,'c-.',label="Diffthermo w/ splines", markersize=1.5)
plt.plot(SOC_julia,np.abs(OCV_diffthermo-OCV_julia),'b->',label="Monotonic RK", markersize=1.5)
plt.plot(SOC_RK,np.abs(OCV_diffthermo-OCV_RK),'g-.',label="Regular RK", markersize=1.5)
plt.plot(SOC_Plett,np.abs(OCV_diffthermo-OCV_Plett),'m-',label="Skewed RK", markersize=1.5)
plt.legend(fontsize=10, frameon=False)
plt.xlim([0.0,1.0])
plt.yscale("log")
plt.xlabel("SOC", fontsize=14)
plt.xticks(fontsize=14)
plt.ylabel("Error (V)", fontsize=14)
plt.yticks(fontsize=14)
# plt.tight_layout()
for axis in ['top','bottom','left','right']:
    ax.spines[axis].set_linewidth(2)
ax.tick_params(width=2)
plt.savefig('error_plot_on_log_scale.png', dpi=200, bbox_inches='tight') 
plt.close()
